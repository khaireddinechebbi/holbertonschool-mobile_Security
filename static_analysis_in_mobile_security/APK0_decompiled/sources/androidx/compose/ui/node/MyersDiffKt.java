...
public final class MyersDiffKt {
    private static final IntStack calculateDiff(int oldSize, int newSize, DiffCallback cb) {
        ...
        int[] forward = CenteredArray.m4327constructorimpl(new int[(oldEnd * 2) + 1]);
        int[] backward = CenteredArray.m4327constructorimpl(new int[(oldEnd * 2) + 1]);
        int[] snake2 = Snake.m4502constructorimpl(new int[5]);
        while (stack.isNotEmpty()) {
            ...
            int[] snake3 = snake2;
            boolean found = m4404midPointq5eDKzI(oldStart, oldEnd2, newStart, newEnd, cb, forward, backward, snake2);
            if (!found) {
                snake2 = snake3;
                oldEnd = max;
                forward = forward2;
            } else {
                if (Snake.m4505getDiagonalSizeimpl(snake3) <= 0) {
                    snake = snake3;
                } else {
                    snake = snake3;
                    Snake.m4500addDiagonalToStackimpl(snake, diagonals);
                }
                stack.pushRange(oldStart, Snake.m4510getStartXimpl(snake), newStart, Snake.m4511getStartYimpl(snake));
                stack.pushRange(Snake.m4506getEndXimpl(snake), oldEnd2, Snake.m4507getEndYimpl(snake), newEnd);
                snake2 = snake;
                oldEnd = max;
                forward = forward2;
            }
    ...
    private static final boolean m4404midPointq5eDKzI(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int[] snake) {
        ...
        CenteredArray.m4333setimpl(forward, 1, oldStart);
        CenteredArray.m4333setimpl(backward, 1, oldEnd);
        int d = 0;
        while (d < max) {
            int d2 = d;
            boolean found = m4403forward4l5_RBY(oldStart, oldEnd, newStart, newEnd, cb, forward, backward, d2, snake);
            if (found) {
                return true;
            }
            boolean found2 = m4402backward4l5_RBY(oldStart, oldEnd, newStart, newEnd, cb, forward, backward, d2, snake);
            if (found2) {
                return true;
            }
            d = d2 + 1;
    ...
    private static final boolean m4403forward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int d, int[] snake) {
        int startX;
        int x;
        int i = oldEnd;
        ...
        int k = -d;
        while (k <= d) {
            if (k == (-d) || (k != d && CenteredArray.m4330getimpl(forward, k + 1) > CenteredArray.m4330getimpl(forward, k - 1))) {
                startX = CenteredArray.m4330getimpl(forward, k + 1);
                x = startX;
            } else {
                startX = CenteredArray.m4330getimpl(forward, k - 1);
                x = startX + 1;
            ...
            CenteredArray.m4333setimpl(forward, k, x);
            if (checkForSnake) {
                int backwardsK = delta - k;
                if (backwardsK >= (-d) + i2 && backwardsK <= d - 1) {
                    if (CenteredArray.m4330getimpl(backward, backwardsK) > x) {
                        i2 = 1;
                    } else {
    ...
    private static final boolean m4402backward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int d, int[] snake) {
        int startX;
        int x;
        int oldSize = oldEnd - oldStart;
        int newSize = newEnd - newStart;
        ...
        int delta = oldSize - newSize;
        for (int k = -d; k <= d; k += 2) {
            if (k == (-d) || (k != d && CenteredArray.m4330getimpl(backward, k + 1) < CenteredArray.m4330getimpl(backward, k - 1))) {
                startX = CenteredArray.m4330getimpl(backward, k + 1);
                x = startX;
            } else {
                startX = CenteredArray.m4330getimpl(backward, k - 1);
                x = startX - 1;
            ...
            CenteredArray.m4333setimpl(backward, k, x);
            if (checkForSnake) {
                int forwardsK = delta - k;
                if (forwardsK >= (-d) && forwardsK <= d && CenteredArray.m4330getimpl(forward, forwardsK) >= x) {
