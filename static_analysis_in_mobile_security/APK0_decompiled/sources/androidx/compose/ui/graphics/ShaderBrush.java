...
public abstract class ShaderBrush extends Brush {
    ...
    public abstract Shader mo2940createShaderuvyYCjk(long size);

    public ShaderBrush() {
        super(null);
        this.createdSize = Size.INSTANCE.m2808getUnspecifiedNHjbRc();
    }

    @Override // androidx.compose.ui.graphics.Brush
    /* renamed from: applyTo-Pq9zytI */
    public final void mo2918applyToPq9zytI(long size, Paint p, float alpha) {
        Intrinsics.checkNotNullParameter(p, "p");
        Shader shader = this.internalShader;
        if (shader == null || !Size.m2796equalsimpl0(this.createdSize, size)) {
            if (Size.m2802isEmptyimpl(size)) {
                shader = null;
                this.internalShader = null;
                this.createdSize = Size.INSTANCE.m2808getUnspecifiedNHjbRc();
            } else {
                Shader it = mo2940createShaderuvyYCjk(size);
                this.internalShader = it;
                shader = it;
                this.createdSize = size;
            }
        }
        if (!Color.m2972equalsimpl0(p.mo2846getColor0d7_KjU(), Color.INSTANCE.m2997getBlack0d7_KjU())) {
            p.mo2852setColor8_81llA(Color.INSTANCE.m2997getBlack0d7_KjU());
        }
