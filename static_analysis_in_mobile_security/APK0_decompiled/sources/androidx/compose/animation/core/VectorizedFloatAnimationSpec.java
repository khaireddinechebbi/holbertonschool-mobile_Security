...
public final class VectorizedFloatAnimationSpec<V extends AnimationVector> implements VectorizedFiniteAnimationSpec<V> {
    ...
    public long getDurationNanos(V initialValue, V targetValue, V initialVelocity) {
        ...
        Intrinsics.checkNotNullParameter(initialVelocity, "initialVelocity");
        long maxDuration = 0;
        Iterable $this$forEach$iv = RangesKt.until(0, initialValue.getSize());
        Iterator<Integer> it = $this$forEach$iv.iterator();
        while (it.hasNext()) {
            int element$iv = ((IntIterator) it).nextInt();
            maxDuration = Math.max(maxDuration, this.anims.get(element$iv).getDurationNanos(initialValue.get$animation_core_release(element$iv), targetValue.get$animation_core_release(element$iv), initialVelocity.get$animation_core_release(element$iv)));
        }
        return maxDuration;
    }
}
