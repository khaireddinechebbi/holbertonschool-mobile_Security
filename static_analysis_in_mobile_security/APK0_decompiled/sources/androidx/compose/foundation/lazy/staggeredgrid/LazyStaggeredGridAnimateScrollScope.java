...
public final class LazyStaggeredGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        long it = findVisibleItem.getOffset();
        return Integer.valueOf(this.state.getIsVertical() ? IntOffset.m5393getYimpl(it) : IntOffset.m5392getXimpl(it));
    }
    ...
    public float expectedDistanceTo(int index, int targetScrollOffset) {
        ...
        for (int index$iv$iv = 0; index$iv$iv < size; index$iv$iv++) {
            ...
            sum$iv += this.state.getIsVertical() ? IntSize.m5433getHeightimpl(it.getSize()) : IntSize.m5434getWidthimpl(it.getSize());
        }
        int itemSizeSum = sum$iv;
        int averageMainAxisItemSize = (itemSizeSum / visibleItems.size()) + layoutInfo.getMainAxisItemSpacing();
