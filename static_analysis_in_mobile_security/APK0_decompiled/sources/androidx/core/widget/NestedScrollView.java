...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (velocityTracker2 != null) {
            velocityTracker2.addMovement(velocityTrackerMotionEvent);
        }
    ...
    private int scrollBy(int verticalScrollDistance, int x, int touchType, boolean isSourceMouseOrKeyboard) {
        ...
        iArr[1] = 0;
        dispatchNestedScroll(0, scrollYDelta, 0, unconsumedY, this.mScrollOffset, touchType, iArr);
        ...
        if (hitScrollBarrier && touchType == 0) {
    ...
    int consumeFlingInVerticalStretch(int unconsumedY) {
        ...
        if (unconsumedY > 0 && EdgeEffectCompat.getDistance(this.mEdgeGlowTop) != 0.0f) {
            float deltaDistance = ((-unconsumedY) * 4.0f) / height;
            int consumed = Math.round(((-height) / 4.0f) * EdgeEffectCompat.onPullDistance(this.mEdgeGlowTop, deltaDistance, 0.5f));
        ...
        if (unconsumedY < 0 && EdgeEffectCompat.getDistance(this.mEdgeGlowBottom) != 0.0f) {
            float deltaDistance2 = (unconsumedY * 4.0f) / height;
            int consumed2 = Math.round((height / 4.0f) * EdgeEffectCompat.onPullDistance(this.mEdgeGlowBottom, deltaDistance2, 0.5f));
    ...
    private float getSplineFlingDistance(int velocity) {
        ...
        return (float) (this.mPhysicalCoeff * SCROLL_FRICTION * Math.exp((f / decelMinusOne) * l));
    }
    ...
    public boolean onGenericMotionEvent(MotionEvent motionEvent) {
        ...
        if (motionEvent.getAction() == 8 && !this.mIsBeingDragged) {
            ...
            if (verticalScroll != 0.0f) {
                ...
                boolean isSourceMouse = MotionEventCompat.isFromSource(motionEvent, InputDeviceCompat.SOURCE_MOUSE);
