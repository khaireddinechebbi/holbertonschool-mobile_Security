...
public final class TextMeasurer {
    ...
    public TextMeasurer(FontFamily.Resolver fallbackFontFamilyResolver, Density fallbackDensity, LayoutDirection fallbackLayoutDirection, int cacheSize) {
        ...
        if (cacheSize > 0) {
            textLayoutCache = new TextLayoutCache(cacheSize);
        } else {
    ...
    public static /* synthetic */ TextLayoutResult m4744measurexDpz5zY$default(TextMeasurer textMeasurer, AnnotatedString annotatedString, TextStyle textStyle, int i, boolean z, int i2, List list, long j, LayoutDirection layoutDirection, Density density, FontFamily.Resolver resolver, boolean z2, int i3, Object obj) {
        return textMeasurer.m4746measurexDpz5zY(annotatedString, (i3 & 2) != 0 ? TextStyle.INSTANCE.getDefault() : textStyle, (i3 & 4) != 0 ? TextOverflow.INSTANCE.m5181getClipgIe3tQ8() : i, (i3 & 8) != 0 ? true : z, (i3 & 16) != 0 ? Integer.MAX_VALUE : i2, (i3 & 32) != 0 ? CollectionsKt.emptyList() : list, (i3 & 64) != 0 ? ConstraintsKt.Constraints$default(0, 0, 0, 0, 15, null) : j, (i3 & 128) != 0 ? textMeasurer.fallbackLayoutDirection : layoutDirection, (i3 & 256) != 0 ? textMeasurer.fallbackDensity : density, (i3 & 512) != 0 ? textMeasurer.fallbackFontFamilyResolver : resolver, (i3 & 1024) != 0 ? false : z2);
    }

    /* renamed from: measure-xDpz5zY, reason: not valid java name */
    public final TextLayoutResult m4746measurexDpz5zY(AnnotatedString text, TextStyle style, int overflow, boolean softWrap, int maxLines, List<AnnotatedString.Range<Placeholder>> placeholders, long constraints, LayoutDirection layoutDirection, Density density, FontFamily.Resolver fontFamilyResolver, boolean skipCache) {
        ...
        if (!skipCache && (textLayoutCache = this.textLayoutCache) != null) {
            cacheResult = textLayoutCache.get(requestedTextLayoutInput);
        } else {
            cacheResult = null;
        }
        if (cacheResult != null) {
            return cacheResult.m4739copyO0kMr_c(requestedTextLayoutInput, ConstraintsKt.m5229constrain4WqzIAM(constraints, IntSizeKt.IntSize(ParagraphKt.ceilToInt(cacheResult.getMultiParagraph().getWidth()), ParagraphKt.ceilToInt(cacheResult.getMultiParagraph().getHeight()))));
        }
        TextLayoutResult it = INSTANCE.layout(requestedTextLayoutInput);
        TextLayoutCache textLayoutCache2 = this.textLayoutCache;
        if (textLayoutCache2 != null) {
            textLayoutCache2.put(requestedTextLayoutInput, it);
            return it;
        }
        return it;
    }
    ...
    public final TextLayoutResult m4745measurewNUYSr0(String text, TextStyle style, int overflow, boolean softWrap, int maxLines, long constraints, LayoutDirection layoutDirection, Density density, FontFamily.Resolver fontFamilyResolver, boolean skipCache) {
        Intrinsics.checkNotNullParameter(text, "text");
        Intrinsics.checkNotNullParameter(style, "style");
        Intrinsics.checkNotNullParameter(layoutDirection, "layoutDirection");
        Intrinsics.checkNotNullParameter(density, "density");
        Intrinsics.checkNotNullParameter(fontFamilyResolver, "fontFamilyResolver");
        return m4744measurexDpz5zY$default(this, new AnnotatedString(text, null, null, 6, null), style, overflow, softWrap, maxLines, null, constraints, layoutDirection, density, fontFamilyResolver, skipCache, 32, null);
    }
    ...
    public static final class Companion {
        ...
        public final TextLayoutResult layout(TextLayoutInput textLayoutInput) {
            ...
            int minWidth = Constraints.m5220getMinWidthimpl(textLayoutInput.getConstraints());
            boolean overwriteMaxLines = false;
            boolean widthMatters = textLayoutInput.getSoftWrap() || TextOverflow.m5174equalsimpl0(textLayoutInput.getOverflow(), TextOverflow.INSTANCE.m5182getEllipsisgIe3tQ8());
            if (widthMatters && Constraints.m5214getHasBoundedWidthimpl(textLayoutInput.getConstraints())) {
                maxWidth = Constraints.m5218getMaxWidthimpl(textLayoutInput.getConstraints());
            } else {
                maxWidth = Integer.MAX_VALUE;
            }
            if (!textLayoutInput.getSoftWrap() && TextOverflow.m5174equalsimpl0(textLayoutInput.getOverflow(), TextOverflow.INSTANCE.m5182getEllipsisgIe3tQ8())) {
            ...
            MultiParagraph multiParagraph = new MultiParagraph(nonNullIntrinsics, ConstraintsKt.Constraints$default(0, width, 0, Constraints.m5217getMaxHeightimpl(textLayoutInput.getConstraints()), 5, null), finalMaxLines, TextOverflow.m5174equalsimpl0(textLayoutInput.getOverflow(), TextOverflow.INSTANCE.m5182getEllipsisgIe3tQ8()), null);
            return new TextLayoutResult(textLayoutInput, multiParagraph, ConstraintsKt.m5229constrain4WqzIAM(textLayoutInput.getConstraints(), IntSizeKt.IntSize((int) Math.ceil(multiParagraph.getWidth()), (int) Math.ceil(multiParagraph.getHeight()))), null);
        }
    }
}
