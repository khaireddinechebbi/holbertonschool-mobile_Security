...
public final class OutlineResolver {
    ...
    public OutlineResolver(Density density) {
        ...
        this.size = Size.INSTANCE.m2809getZeroNHjbRc();
        this.shape = RectangleShapeKt.getRectangleShape();
        this.rectTopLeft = Offset.INSTANCE.m2747getZeroF1C5BW0();
        this.rectSize = Size.INSTANCE.m2809getZeroNHjbRc();
    ...
    public final boolean m4582isInOutlinek4lQ0M(long position) {
        androidx.compose.ui.graphics.Outline outline;
        if (this.outlineNeeded && (outline = this.calculatedOutline) != null) {
            return ShapeContainingUtilKt.isInOutline(outline, Offset.m2731getXimpl(position), Offset.m2732getYimpl(position), this.tmpTouchPointPath, this.tmpOpPath);
        }
        return true;
    }
    ...
    public final void clipToOutline(Canvas canvas) {
        ...
        if (targetPath != null) {
            Canvas.CC.m2943clipPathmtrdDE$default(canvas, targetPath, 0, 2, null);
            return;
        }
        ...
        if (f > 0.0f) {
            ...
            if (roundRectClipPath == null || !m4581isSameBounds4L21HEs(roundRect, this.rectTopLeft, this.rectSize, f)) {
                RoundRect roundRect2 = RoundRectKt.m2785RoundRectgG7oq9Y(Offset.m2731getXimpl(this.rectTopLeft), Offset.m2732getYimpl(this.rectTopLeft), Offset.m2731getXimpl(this.rectTopLeft) + Size.m2800getWidthimpl(this.rectSize), Offset.m2732getYimpl(this.rectTopLeft) + Size.m2797getHeightimpl(this.rectSize), CornerRadiusKt.CornerRadius$default(this.roundedCornerRadius, 0.0f, 2, null));
            ...
            Canvas.CC.m2943clipPathmtrdDE$default(canvas, roundRectClipPath, 0, 2, null);
            return;
        }
        Canvas.CC.m2944clipRectN_I0leg$default(canvas, Offset.m2731getXimpl(this.rectTopLeft), Offset.m2732getYimpl(this.rectTopLeft), Offset.m2731getXimpl(this.rectTopLeft) + Size.m2800getWidthimpl(this.rectSize), Offset.m2732getYimpl(this.rectTopLeft) + Size.m2797getHeightimpl(this.rectSize), 0, 16, null);
    }

    /* renamed from: update-uvyYCjk, reason: not valid java name */
    public final void m4583updateuvyYCjk(long size) {
        if (!Size.m2796equalsimpl0(this.size, size)) {
            this.size = size;
            this.cacheIsDirty = true;
        }
    }
    ...
    private final void updateCache() {
        if (this.cacheIsDirty) {
            ...
            if (this.outlineNeeded && Size.m2800getWidthimpl(j) > 0.0f && Size.m2797getHeightimpl(this.size) > 0.0f) {
                this.isSupportedOutline = true;
                androidx.compose.ui.graphics.Outline outline = this.shape.mo209createOutlinePq9zytI(this.size, this.layoutDirection, this.density);
    ...
    private final void updateCacheWithRoundRect(RoundRect roundRect) {
        float radius = CornerRadius.m2706getXimpl(roundRect.m2781getTopLeftCornerRadiuskKHJgLs());
    ...
    private final boolean m4581isSameBounds4L21HEs(RoundRect $this$isSameBounds_u2d4L21HEs, long offset, long size, float radius) {
        if ($this$isSameBounds_u2d4L21HEs == null || !RoundRectKt.isSimple($this$isSameBounds_u2d4L21HEs)) {
            return false;
        }
        if (!($this$isSameBounds_u2d4L21HEs.getLeft() == Offset.m2731getXimpl(offset))) {
            return false;
        }
        if (!($this$isSameBounds_u2d4L21HEs.getTop() == Offset.m2732getYimpl(offset))) {
            return false;
        }
        if (!($this$isSameBounds_u2d4L21HEs.getRight() == Offset.m2731getXimpl(offset) + Size.m2800getWidthimpl(size))) {
            return false;
        }
        if ($this$isSameBounds_u2d4L21HEs.getBottom() == Offset.m2732getYimpl(offset) + Size.m2797getHeightimpl(size)) {
            return (CornerRadius.m2706getXimpl($this$isSameBounds_u2d4L21HEs.m2781getTopLeftCornerRadiuskKHJgLs()) > radius ? 1 : (CornerRadius.m2706getXimpl($this$isSameBounds_u2d4L21HEs.m2781getTopLeftCornerRadiuskKHJgLs()) == radius ? 0 : -1)) == 0;
        }
        return false;
    }
}
