...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl create(MagnifierStyle style, View view, Density density, float initialZoom) {
        ...
        if (!Intrinsics.areEqual(style, MagnifierStyle.INSTANCE.getTextDefault())) {
            long size = density.mo328toSizeXkaWNTQ(style.getSize());
            float cornerRadius = density.mo327toPx0680j_4(style.getCornerRadius());
            float elevation = density.mo327toPx0680j_4(style.getElevation());
            Magnifier.Builder $this$create_u24lambda_u241_u24lambda_u240 = new Magnifier.Builder(view);
            if (size != Size.INSTANCE.m2808getUnspecifiedNHjbRc()) {
                $this$create_u24lambda_u241_u24lambda_u240.setSize(MathKt.roundToInt(Size.m2800getWidthimpl(size)), MathKt.roundToInt(Size.m2797getHeightimpl(size)));
            }
            ...
            $this$create_u24lambda_u241_u24lambda_u240.setClippingEnabled(style.getClippingEnabled());
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo244updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m2750isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m2731getXimpl(sourceCenter), Offset.m2732getYimpl(sourceCenter), Offset.m2731getXimpl(magnifierCenter), Offset.m2732getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m2731getXimpl(sourceCenter), Offset.m2732getYimpl(sourceCenter));
            }
        }
    }
}
