...
public final class CalendarModelImpl implements CalendarModel {
    ...
    public CalendarModelImpl() {
        Locale $this$weekdayNames_u24lambda_u241 = Locale.getDefault();
        DayOfWeek[] values = DayOfWeek.values();
        Collection destination$iv$iv = new ArrayList(values.length);
        for (DayOfWeek dayOfWeek : values) {
            destination$iv$iv.add(TuplesKt.to(dayOfWeek.getDisplayName(TextStyle.FULL, $this$weekdayNames_u24lambda_u241), dayOfWeek.getDisplayName(TextStyle.NARROW, $this$weekdayNames_u24lambda_u241)));
        }
    ...
    public static final class Companion {
        ...
        public final String formatWithPattern(long utcTimeMillis, String pattern, Locale locale) {
            Intrinsics.checkNotNullParameter(pattern, "pattern");
            Intrinsics.checkNotNullParameter(locale, "locale");
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern(pattern, locale).withDecimalStyle(DecimalStyle.of(locale));
            Intrinsics.checkNotNullExpressionValue(formatter, "ofPattern(pattern, local…(DecimalStyle.of(locale))");
            String format = Instant.ofEpochMilli(utcTimeMillis).atZone(getUtcTimeZoneId$material3_release()).toLocalDate().format(formatter);
    ...
    private final LocalDate toLocalDate(CalendarDate $this$toLocalDate) {
        LocalDate of = LocalDate.of($this$toLocalDate.getYear(), $this$toLocalDate.getMonth(), $this$toLocalDate.getDayOfMonth());
