...
public final class MultiWidgetSelectionDelegate implements Selectable {
    ...
    private final synchronized int getLastVisibleOffset(TextLayoutResult $this$lastVisibleOffset) {
        ...
        if (this._previousTextLayoutResult != $this$lastVisibleOffset) {
            if ($this$lastVisibleOffset.getDidOverflowHeight() && !$this$lastVisibleOffset.getMultiParagraph().getDidExceedMaxLines()) {
                finalVisibleLine = RangesKt.coerceAtMost($this$lastVisibleOffset.getLineForVerticalPosition(IntSize.m5433getHeightimpl($this$lastVisibleOffset.getSize())), $this$lastVisibleOffset.getLineCount() - 1);
                while ($this$lastVisibleOffset.getLineTop(finalVisibleLine) >= IntSize.m5433getHeightimpl($this$lastVisibleOffset.getSize())) {
                    finalVisibleLine--;
                }
    ...
    public Pair<Selection, Boolean> mo892updateSelectionqCDeeow(long startHandlePosition, long endHandlePosition, Offset previousHandlePosition, boolean isStartHandle, LayoutCoordinates containerLayoutCoordinates, SelectionAdjustment adjustment, Selection previousSelection) {
        ...
        if (layoutCoordinates != null && (textLayoutResult = this.layoutResultCallback.invoke()) != null) {
            long relativePosition = containerLayoutCoordinates.mo4233localPositionOfR5De75A(layoutCoordinates, Offset.INSTANCE.m2747getZeroF1C5BW0());
            long localStartPosition = Offset.m2735minusMKHz9U(startHandlePosition, relativePosition);
            long localEndPosition = Offset.m2735minusMKHz9U(endHandlePosition, relativePosition);
            if (previousHandlePosition != null) {
                long it = previousHandlePosition.getPackedValue();
                offset = Offset.m2720boximpl(Offset.m2735minusMKHz9U(it, relativePosition));
            }
            Offset localPreviousHandlePosition = offset;
            return MultiWidgetSelectionDelegateKt.m896getTextSelectionInfoyM0VcXU(textLayoutResult, localStartPosition, localEndPosition, localPreviousHandlePosition, getSelectableId(), adjustment, previousSelection, isStartHandle);
        }
    ...
    public Selection getSelectAllSelection() {
        Selection m894getAssembledSelectionInfovJH6DeI;
        ...
        m894getAssembledSelectionInfovJH6DeI = MultiWidgetSelectionDelegateKt.m894getAssembledSelectionInfovJH6DeI(newSelectionRange, false, getSelectableId(), textLayoutResult);
        return m894getAssembledSelectionInfovJH6DeI;
    }

    @Override // androidx.compose.foundation.text.selection.Selectable
    /* renamed from: getHandlePosition-dBAh8RU, reason: not valid java name */
    public long mo890getHandlePositiondBAh8RU(Selection selection, boolean isStartHandle) {
        TextLayoutResult textLayoutResult;
    ...
    public long mo891getRangeOfLineContainingjx7JFs(int offset) {
