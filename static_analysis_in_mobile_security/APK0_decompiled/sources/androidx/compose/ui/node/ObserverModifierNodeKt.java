...
public final class ObserverModifierNodeKt {
    public static final <T extends Modifier.Node & ObserverModifierNode> void observeReads(T t, Function0<Unit> block) {
        ...
        ObserverNodeOwnerScope it = t.getOwnerScope();
        if (it == null) {
            it = new ObserverNodeOwnerScope(t);
            t.setOwnerScope$ui_release(it);
        }
        DelegatableNodeKt.requireOwner(t).getSnapshotObserver().observeReads$ui_release(it, ObserverNodeOwnerScope.INSTANCE.getOnObserveReadsChanged$ui_release(), block);
    }
}
