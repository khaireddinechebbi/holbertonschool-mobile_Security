...
public final class RippleHostView extends View {
    ...
    public final void m1295addRippleKOepWvA(PressInteraction.Press interaction, boolean bounded, long size, int radius, long color, float alpha, Function0<Unit> onInvalidateRipple) {
        ...
        m1296updateRipplePropertiesbiQXAtU(size, radius, color, alpha);
        if (bounded) {
            ripple.setHotspot(Offset.m2731getXimpl(interaction.getPressPosition()), Offset.m2732getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m1296updateRipplePropertiesbiQXAtU(long size, int radius, long color, float alpha) {
        UnprojectedRipple ripple = this.ripple;
        if (ripple == null) {
            return;
        }
        ripple.trySetRadius(radius);
        ripple.m1303setColorDxMtmZc(color, alpha);
        Rect newBounds = new Rect(0, 0, MathKt.roundToInt(Size.m2800getWidthimpl(size)), MathKt.roundToInt(Size.m2797getHeightimpl(size)));
