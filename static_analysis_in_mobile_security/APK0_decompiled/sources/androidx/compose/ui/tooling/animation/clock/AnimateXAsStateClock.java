...
public final class AnimateXAsStateClock<T, V extends AnimationVector> implements ComposeAnimationClock<AnimateXAsStateComposeAnimation<T, V>, TargetState<T>> {
    ...
    public AnimateXAsStateClock(AnimateXAsStateComposeAnimation<T, V> animation) {
        ...
        this.state = new TargetState<>(getAnimation().m5188getAnimationObject().getValue(), getAnimation().m5188getAnimationObject().getValue());
    ...
    private final TargetBasedAnimation<T, V> getCurrentAnimation() {
        return AnimationKt.TargetBasedAnimation(getAnimation().getAnimationSpec(), getAnimation().m5188getAnimationObject().getTypeConverter(), getState().getInitial(), getState().getTarget(), getAnimation().m5188getAnimationObject().getVelocity());
    }
}
