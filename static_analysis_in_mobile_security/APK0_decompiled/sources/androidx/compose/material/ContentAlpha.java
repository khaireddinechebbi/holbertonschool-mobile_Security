...
public final class ContentAlpha {
    ...
    private final float contentAlpha(float highContrastAlpha, float lowContrastAlpha, Composer $composer, int $changed) {
        ...
        long contentColor = ((Color) consume).m2981unboximpl();
        boolean lightTheme = MaterialTheme.INSTANCE.getColors($composer, 6).isLight();
        float f = (!lightTheme ? ((double) ColorKt.m3023luminance8_81llA(contentColor)) < 0.5d : ((double) ColorKt.m3023luminance8_81llA(contentColor)) > 0.5d) ? lowContrastAlpha : highContrastAlpha;
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventEnd();
        }
        $composer.endReplaceableGroup();
