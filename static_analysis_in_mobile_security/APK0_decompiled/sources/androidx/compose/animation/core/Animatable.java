...
public final class Animatable<T, V extends AnimationVector> {
    ...
    private final V createVector(T t, float value) {
        V invoke = this.typeConverter.getConvertToVector().invoke(t);
        int size = invoke.getSize();
        for (int i = 0; i < size; i++) {
            invoke.set$animation_core_release(i, value);
        }
        return invoke;
    }
    ...
    public final void updateBounds(T lowerBound, T upperBound) {
        ...
        int size = v3.getSize();
        for (int i = 0; i < size; i++) {
            if (!(v3.get$animation_core_release(i) <= v4.get$animation_core_release(i))) {
                throw new IllegalStateException(("Lower bound must be no greater than upper bound on *all* dimensions. The provided lower bound: " + v3 + " is greater than upper bound " + v4 + " on index " + i).toString());
            }
        }
