...
public final class ShaderBrushSpan extends CharacterStyle implements UpdateAppearance {
    ...
    public ShaderBrushSpan(ShaderBrush shaderBrush, float alpha) {
        ...
        this.alpha = alpha;
        this.size = Size.INSTANCE.m2808getUnspecifiedNHjbRc();
    }
    ...
    public final void m5026setSizeuvyYCjk(long j) {
        this.size = j;
    }
    ...
    public void updateDrawState(TextPaint textPaint) {
        ...
        if ($this$isUnspecified$iv == Size.INSTANCE.m2808getUnspecifiedNHjbRc()) {
            return;
        }
        Pair finalCachedShader = this.cachedShader;
        if (finalCachedShader == null || !Size.m2796equalsimpl0(finalCachedShader.getFirst().getPackedValue(), this.size)) {
            shader = this.shaderBrush.mo2940createShaderuvyYCjk(this.size);
        } else {
            shader = finalCachedShader.getSecond();
        }
        textPaint.setShader(shader);
        this.cachedShader = TuplesKt.to(Size.m2788boximpl(this.size), shader);
    }
}
