    ...
    private static final boolean calculateIfHorizontallyStacked(List<SemanticsNode> list) {
        ...
        if (list.size() == 0 || list.size() == 1) {
            ...
            for (int i$iv = 0; i$iv < lastIndex; i$iv++) {
                ...
                result$iv.add(Offset.m2720boximpl(OffsetKt.Offset(Math.abs(Offset.m2731getXimpl(el1.getBoundsInRoot().m2761getCenterF1C5BW0()) - Offset.m2731getXimpl(el2.getBoundsInRoot().m2761getCenterF1C5BW0())), Math.abs(Offset.m2732getYimpl(el1.getBoundsInRoot().m2761getCenterF1C5BW0()) - Offset.m2732getYimpl(el2.getBoundsInRoot().m2761getCenterF1C5BW0())))));
                current$iv = next$iv;
            }
        }
        ...
        if ($this$fastZipWithNext$iv.size() == 1) {
            ...
            if (1 <= lastIndex2) {
                while (true) {
                    ...
                    accumulator$iv = Offset.m2720boximpl(Offset.m2736plusMKHz9U(result, element));
                    if (i$iv2 == lastIndex2) {
                        break;
                    }
                    i$iv2++;
        ...
        float deltaX = Offset.m2721component1impl(packedValue);
        float deltaY = Offset.m2722component2impl(packedValue);
