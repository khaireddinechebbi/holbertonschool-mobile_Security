...
public abstract class LazyGridMeasuredItemProvider {
    ...
    public static /* synthetic */ LazyGridMeasuredItem m625getAndMeasure3p2s80s$default(LazyGridMeasuredItemProvider lazyGridMeasuredItemProvider, int i, int i2, long j, int i3, Object obj) {
        if (obj != null) {
        ...
        return lazyGridMeasuredItemProvider.m626getAndMeasure3p2s80s(i, i2, j);
    }

    /* renamed from: getAndMeasure-3p2s80s, reason: not valid java name */
    public final LazyGridMeasuredItem m626getAndMeasure3p2s80s(int index, int mainAxisSpacing, long constraints) {
        int crossAxisSize;
        Object key = this.itemProvider.getKey(index);
        Object contentType = this.itemProvider.getContentType(index);
        List placeables = this.measureScope.mo645measure0kLqBqw(index, constraints);
        if (Constraints.m5216getHasFixedWidthimpl(constraints)) {
            crossAxisSize = Constraints.m5220getMinWidthimpl(constraints);
        } else {
            if (!Constraints.m5215getHasFixedHeightimpl(constraints)) {
                throw new IllegalArgumentException("Failed requirement.".toString());
            }
            crossAxisSize = Constraints.m5219getMinHeightimpl(constraints);
        }
