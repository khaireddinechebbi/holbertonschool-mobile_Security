...
public abstract class CornerBasedShape implements Shape {
    ...
    public abstract Outline mo731createOutlineLjSzlW0(long size, float topStart, float topEnd, float bottomEnd, float bottomStart, LayoutDirection layoutDirection);
    ...
    public final Outline mo209createOutlinePq9zytI(long size, LayoutDirection layoutDirection, Density density) {
        float topStart;
        float bottomStart;
        float topEnd;
        float bottomEnd;
        ...
        float topStart2 = this.topStart.mo739toPxTmRCtEA(size, density);
        float topEnd2 = this.topEnd.mo739toPxTmRCtEA(size, density);
        float bottomEnd2 = this.bottomEnd.mo739toPxTmRCtEA(size, density);
        float bottomStart2 = this.bottomStart.mo739toPxTmRCtEA(size, density);
        float minDimension = Size.m2799getMinDimensionimpl(size);
        if (topStart2 + bottomStart2 > minDimension) {
            float scale = minDimension / (topStart2 + bottomStart2);
            topStart = topStart2 * scale;
            bottomStart = bottomStart2 * scale;
        } else {
            topStart = topStart2;
            bottomStart = bottomStart2;
        }
        if (topEnd2 + bottomEnd2 > minDimension) {
            float scale2 = minDimension / (topEnd2 + bottomEnd2);
            topEnd = topEnd2 * scale2;
            bottomEnd = bottomEnd2 * scale2;
        } else {
            topEnd = topEnd2;
            bottomEnd = bottomEnd2;
        }
        if (topStart >= 0.0f && topEnd >= 0.0f && bottomEnd >= 0.0f && bottomStart >= 0.0f) {
            return mo731createOutlineLjSzlW0(size, topStart, topEnd, bottomEnd, bottomStart, layoutDirection);
        }
        throw new IllegalArgumentException(("Corner size in Px can't be negative(topStart = " + topStart + ", topEnd = " + topEnd + ", bottomEnd = " + bottomEnd + ", bottomStart = " + bottomStart + ")!").toString());
    }
