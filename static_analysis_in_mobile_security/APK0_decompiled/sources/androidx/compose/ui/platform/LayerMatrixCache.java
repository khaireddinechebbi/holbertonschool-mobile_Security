...
public final class LayerMatrixCache<T> {
    ...
    public final float[] m4560calculateMatrixGrdbGEg(T target) {
        float[] it = this.matrixCache;
        if (it == null) {
            it = androidx.compose.ui.graphics.Matrix.m3202constructorimpl$default(null, 1, null);
            this.matrixCache = it;
        }
        if (!this.isDirty) {
            return it;
        ...
        if (prevMatrix == null || !Intrinsics.areEqual(it2, prevMatrix)) {
            AndroidMatrixConversions_androidKt.m2844setFromtUYjHk(it, it2);
    ...
    public final float[] m4559calculateInverseMatrixbWbORWo(T target) {
        ...
        if (this.isInverseDirty) {
            float[] normalMatrix = m4560calculateMatrixGrdbGEg(target);
            this.isInverseValid = InvertMatrixKt.m4557invertToJiSxe2E(normalMatrix, it);
