...
public final class LazyGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        if (item != null) {
            return Integer.valueOf(this.state.getIsVertical() ? IntOffset.m5393getYimpl(item.getOffset()) : IntOffset.m5392getXimpl(item.getOffset()));
        }
        return null;
    }
        ...
        while (lineStartIndex < visibleItems.size()) {
            ...
            if (currentLine == -1) {
                ...
                while (lineEndIndex < visibleItems.size() && lineOf.invoke(Integer.valueOf(lineEndIndex)).intValue() == currentLine) {
                    if (isVertical) {
                        m5434getWidthimpl = IntSize.m5433getHeightimpl(visibleItems.get(lineEndIndex).getSize());
                    } else {
                        m5434getWidthimpl = IntSize.m5434getWidthimpl(visibleItems.get(lineEndIndex).getSize());
                    }
                    lineMainAxisSize = Math.max(lineMainAxisSize, m5434getWidthimpl);
                    lineEndIndex++;
                }
