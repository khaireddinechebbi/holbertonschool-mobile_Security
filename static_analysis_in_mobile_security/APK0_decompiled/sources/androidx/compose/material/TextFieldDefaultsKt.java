...
public final class TextFieldDefaultsKt {
    ...
    public static final State<BorderStroke> m1259animateBorderStrokeAsStateNuRrP5Q(boolean enabled, boolean isError, InteractionSource interactionSource, TextFieldColors colors, float focusedBorderThickness, float unfocusedBorderThickness, Composer $composer, int $changed) {
        ...
        if (enabled) {
            ...
            animatedThickness = AnimateAsStateKt.m80animateDpAsStateAjpBEmI(targetThickness, AnimationSpecKt.tween$default(150, 0, null, 6, null), null, null, $composer, 48, 12);
            $composer.endReplaceableGroup();
        } else {
            $composer.startReplaceableGroup(1685712164);
            ComposerKt.sourceInformation($composer, "849@38446L46");
            animatedThickness = SnapshotStateKt.rememberUpdatedState(Dp.m5272boximpl(unfocusedBorderThickness), $composer, ($changed >> 15) & 14);
            $composer.endReplaceableGroup();
        }
        State<BorderStroke> rememberUpdatedState = SnapshotStateKt.rememberUpdatedState(new BorderStroke(animatedThickness.getValue().m5288unboximpl(), new SolidColor(indicatorColor.getValue().m2981unboximpl(), null), null), $composer, 0);
        if (ComposerKt.isTraceInProgress()) {
