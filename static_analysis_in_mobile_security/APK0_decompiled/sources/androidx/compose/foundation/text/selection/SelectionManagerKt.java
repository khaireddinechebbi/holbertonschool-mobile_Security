...
public final class SelectionManagerKt {
    ...
    private static final long calculateSelectionMagnifierCenterAndroid_O0kMr_c$getMagnifierCenter(SelectionManager $manager, long $magnifierSize, Selection.AnchorInfo anchor, boolean isStartHandle) {
        ...
        if (selectable != null && (containerCoordinates = $manager.getContainerLayoutCoordinates()) != null && (selectableCoordinates = selectable.getLayoutCoordinates()) != null) {
            ...
            Offset m930getCurrentDragPosition_m7T9E = $manager.m930getCurrentDragPosition_m7T9E();
            Intrinsics.checkNotNull(m930getCurrentDragPosition_m7T9E);
            long localDragPosition = selectableCoordinates.mo4233localPositionOfR5De75A(containerCoordinates, m930getCurrentDragPosition_m7T9E.getPackedValue());
            float dragX = Offset.m2731getXimpl(localDragPosition);
            long line = selectable.mo891getRangeOfLineContainingjx7JFs(offset);
            Rect lineMin = selectable.getBoundingBox(TextRange.m4766getMinimpl(line));
            Rect lineMax = selectable.getBoundingBox(RangesKt.coerceAtLeast(TextRange.m4765getMaximpl(line) - 1, TextRange.m4766getMinimpl(line)));
            ...
            if (Math.abs(dragX - maxX) > IntSize.m5434getWidthimpl($magnifierSize) / 2) {
                return Offset.INSTANCE.m2746getUnspecifiedF1C5BW0();
            }
            Rect anchorBounds = selectable.getBoundingBox(offset);
            float centerY = Offset.m2732getYimpl(anchorBounds.m2761getCenterF1C5BW0());
            return containerCoordinates.mo4233localPositionOfR5De75A(selectableCoordinates, OffsetKt.Offset(maxX, centerY));
        }
        return Offset.INSTANCE.m2746getUnspecifiedF1C5BW0();
    }

    /* renamed from: calculateSelectionMagnifierCenterAndroid-O0kMr_c, reason: not valid java name */
    public static final long m941calculateSelectionMagnifierCenterAndroidO0kMr_c(SelectionManager manager, long magnifierSize) {
    ...
    public static final Rect visibleBounds(LayoutCoordinates $this$visibleBounds) {
        ...
        return RectKt.m2769Rect0a9Yr6o($this$visibleBounds.mo4237windowToLocalMKHz9U(boundsInWindow.m2766getTopLeftF1C5BW0()), $this$visibleBounds.mo4237windowToLocalMKHz9U(boundsInWindow.m2760getBottomRightF1C5BW0()));
    }

    /* renamed from: containsInclusive-Uv8p0NA, reason: not valid java name */
    public static final boolean m942containsInclusiveUv8p0NA(Rect containsInclusive, long offset) {
        Intrinsics.checkNotNullParameter(containsInclusive, "$this$containsInclusive");
        float left = containsInclusive.getLeft();
        float right = containsInclusive.getRight();
        float m2731getXimpl = Offset.m2731getXimpl(offset);
        if (left <= m2731getXimpl && m2731getXimpl <= right) {
            float top = containsInclusive.getTop();
            float bottom = containsInclusive.getBottom();
            float m2732getYimpl = Offset.m2732getYimpl(offset);
            if (top <= m2732getYimpl && m2732getYimpl <= bottom) {
                return true;
            }
        }
        return false;
