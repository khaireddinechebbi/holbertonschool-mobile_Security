...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long distance = m4360findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m4355getDistanceimpl(distance) < 0.0f && DistanceAndInLayer.m4357isInLayerimpl(distance);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        long bestDistance = m4360findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m4351compareToS_HNhKs(bestDistance, distanceAndInLayer) > 0;
    }

    /* renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m4360findBestHitDistanceptXAw2c() {
        long bestDistance;
        bestDistance = HitTestResultKt.DistanceAndInLayer(Float.POSITIVE_INFINITY, false);
        int i = this.hitDepth + 1;
        int lastIndex = CollectionsKt.getLastIndex(this);
        if (i <= lastIndex) {
            while (true) {
                long distance = DistanceAndInLayer.m4352constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                bestDistance = DistanceAndInLayer.m4351compareToS_HNhKs(distance, bestDistance) < 0 ? distance : bestDistance;
                if (DistanceAndInLayer.m4355getDistanceimpl(bestDistance) >= 0.0f || !DistanceAndInLayer.m4357isInLayerimpl(bestDistance)) {
                    if (i == lastIndex) {
                        break;
                    }
                    i++;
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        long previousDistance = m4360findBestHitDistanceptXAw2c();
        int previousHitDepth = this.hitDepth;
        this.hitDepth = CollectionsKt.getLastIndex(this);
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(this) && DistanceAndInLayer.m4351compareToS_HNhKs(previousDistance, m4360findBestHitDistanceptXAw2c()) > 0) {
            int fromIndex = this.hitDepth + 1;
            int toIndex = previousHitDepth + 1;
            Object[] objArr = this.values;
            ArraysKt.copyInto(objArr, objArr, toIndex, fromIndex, size());
            long[] jArr = this.distanceFromEdgeAndInLayer;
    ...
    private final class SubList implements List<Modifier.Node>, KMappedMarker {
        ...
        public ListIterator<Modifier.Node> listIterator(int index) {
            HitTestResult hitTestResult = HitTestResult.this;
            int i = this.minIndex;
            return hitTestResult.new HitTestResultIterator(i + index, i, this.maxIndex);
        }
        ...
        public List<Modifier.Node> subList(int fromIndex, int toIndex) {
            HitTestResult hitTestResult = HitTestResult.this;
            int i = this.minIndex;
            return hitTestResult.new SubList(i + fromIndex, i + toIndex);
        }
    }
}
