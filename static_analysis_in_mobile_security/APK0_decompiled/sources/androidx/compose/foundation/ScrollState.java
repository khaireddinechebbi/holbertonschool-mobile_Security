...
public final class ScrollState implements ScrollableState {
    ...
    private final ScrollableState scrollableState = ScrollableStateKt.ScrollableState(new Function1<Float, Float>() { // from class: androidx.compose.foundation.ScrollState$scrollableState$1
        ...
        public final Float invoke(float it) {
            float f;
            f = ScrollState.this.accumulator;
            float absolute = ScrollState.this.getValue() + it + f;
            float newValue = RangesKt.coerceIn(absolute, 0.0f, ScrollState.this.getMaxValue());
            boolean changed = true ^ (absolute == newValue);
            float consumed = newValue - ScrollState.this.getValue();
            int consumedInt = MathKt.roundToInt(consumed);
            ScrollState scrollState = ScrollState.this;
            scrollState.setValue(scrollState.getValue() + consumedInt);
            ScrollState.this.accumulator = consumed - consumedInt;
