...
public class PersistentHashMapBuilderBaseIterator<K, V, T> extends PersistentHashMapBaseIterator<K, V, T> implements Iterator<T>, KMutableIterator {
    ...
    private final void resetPath(int keyHash, TrieNode<?, ?> node, K key, int pathIndex) {
        ...
        if (node.hasEntryAt$runtime_release(keyPositionMask)) {
            int keyIndex = node.entryKeyIndex$runtime_release(keyPositionMask);
            getPath()[pathIndex].reset(node.getBuffer(), node.entryCount$runtime_release() * 2, keyIndex);
            setPathLastIndex(pathIndex);
        } else {
            int nodeIndex = node.nodeIndex$runtime_release(keyPositionMask);
            TrieNode targetNode = node.nodeAtIndex$runtime_release(nodeIndex);
            getPath()[pathIndex].reset(node.getBuffer(), node.entryCount$runtime_release() * 2, nodeIndex);
