...
final class SplineBasedFloatDecayAnimationSpec implements FloatDecayAnimationSpec {
    ...
    public final float flingDistance(float velocity) {
        ...
        return ((float) (d3 * Math.exp((d / d2) * l))) * Math.signum(velocity);
    }
    ...
    public float getValueFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        ...
        float splinePos = duration > 0 ? playTimeNanos / duration : 1.0f;
        float distance = flingDistance(initialVelocity);
        return (AndroidFlingSpline.FlingResult.m389getDistanceCoefficientimpl(AndroidFlingSpline.INSTANCE.m384flingPositionLfoxSSI(splinePos)) * distance) + initialValue;
    }
    ...
    public long getDurationNanos(float initialValue, float initialVelocity) {
        ...
        return (long) (Math.exp(l / d) * 1.0E9d);
    }
    ...
    public float getVelocityFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        ...
        float splinePos = duration > 0 ? playTimeNanos / duration : 1.0f;
        float distance = flingDistance(initialVelocity);
        return ((AndroidFlingSpline.FlingResult.m390getVelocityCoefficientimpl(AndroidFlingSpline.INSTANCE.m384flingPositionLfoxSSI(splinePos)) * distance) / duration) * 1.0E9f;
    }
}
