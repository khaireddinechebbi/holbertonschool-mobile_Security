...
public class LockFreeLinkedListNode {
    ...
    private final Removed removed() {
        ...
        atomicReferenceFieldUpdater.lazySet(this, it);
        return it;
    }
    ...
    */
    ...
    private final LockFreeLinkedListNode correctPrev(OpDescriptor op) {
        while (true) {
            ...
            while (true) {
                AtomicReferenceFieldUpdater atomicReferenceFieldUpdater = _next$FU;
                Object prevNext = atomicReferenceFieldUpdater.get(prev);
                if (prevNext == this) {
                    if (oldPrev == prev) {
                        return prev;
                    }
                    ...
                    if (prevNext instanceof Removed) {
                        if (last != null) {
                            if (!AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, last, prev, ((Removed) prevNext).ref)) {
                                break;
                            }
                            prev = last;
                            last = null;
