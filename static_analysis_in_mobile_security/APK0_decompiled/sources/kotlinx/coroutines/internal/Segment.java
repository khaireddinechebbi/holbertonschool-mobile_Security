...
public abstract class Segment<S extends Segment<S>> extends ConcurrentLinkedListNode<S> implements NotCompleted {
    ...
    public final boolean tryIncPointers$kotlinx_coroutines_core() {
        int cur$iv;
        AtomicIntegerFieldUpdater atomicfu$handler$iv = cleanedAndPointers$FU;
        do {
            cur$iv = atomicfu$handler$iv.get(this);
            int it = (cur$iv != getNumberOfSlots() || isTail()) ? 1 : 0;
            if (it == 0) {
                return false;
            }
        } while (!atomicfu$handler$iv.compareAndSet(this, cur$iv, cur$iv + 65536));
        return true;
    }
