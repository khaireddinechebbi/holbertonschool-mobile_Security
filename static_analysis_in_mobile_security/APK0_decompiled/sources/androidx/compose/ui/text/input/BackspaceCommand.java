...
public final class BackspaceCommand implements EditCommand {
    ...
    public void applyTo(EditingBuffer buffer) {
        ...
        if (buffer.getCursor$ui_text_release() == -1) {
            ...
            buffer.delete$ui_text_release(delStart, delEnd);
            return;
        }
        int delStart2 = buffer.getCursor$ui_text_release();
        if (delStart2 == 0) {
            return;
        }
        int prevCursorPos = JvmCharHelpers_androidKt.findPrecedingBreak(buffer.toString(), buffer.getCursor$ui_text_release());
        buffer.delete$ui_text_release(prevCursorPos, buffer.getCursor$ui_text_release());
    }
