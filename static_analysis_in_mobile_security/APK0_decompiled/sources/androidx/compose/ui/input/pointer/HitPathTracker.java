...
public final class HitPathTracker {
    ...
    public final void m4035addHitPathKNwqfcY(long pointerId, List<? extends Modifier.Node> pointerInputNodes) {
        Node node;
        Object item$iv;
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (merging) {
                ...
                if (node != null) {
                    node.markIsIn();
                    if (!node.getPointerIds().contains(PointerId.m4086boximpl(pointerId))) {
                        node.getPointerIds().add(PointerId.m4086boximpl(pointerId));
                    }
