...
public abstract class Ripple implements Indication {
    ...
    public final IndicationInstance rememberUpdatedInstance(InteractionSource interactionSource, Composer $composer, int $changed) {
        long mo1133defaultColorWaAFU9c;
        ...
        if ($this$isSpecified$iv != Color.INSTANCE.m3007getUnspecified0d7_KjU()) {
            mo1133defaultColorWaAFU9c = this.color.getValue().m2981unboximpl();
        } else {
            mo1133defaultColorWaAFU9c = theme.mo1133defaultColorWaAFU9c($composer, 0);
        }
        $composer.endReplaceableGroup();
        State color = SnapshotStateKt.rememberUpdatedState(Color.m2961boximpl(mo1133defaultColorWaAFU9c), $composer, 0);
        State rippleAlpha = SnapshotStateKt.rememberUpdatedState(theme.rippleAlpha($composer, 0), $composer, 0);
        RippleIndicationInstance instance = mo1289rememberUpdatedRippleInstance942rkJo(interactionSource, this.bounded, this.radius, color, rippleAlpha, $composer, ($changed & 14) | (($changed << 12) & 458752));
