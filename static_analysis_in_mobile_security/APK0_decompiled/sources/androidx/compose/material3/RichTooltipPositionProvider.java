...
final /* data */ class RichTooltipPositionProvider implements PopupPositionProvider {
    ...
    public RichTooltipPositionProvider(int tooltipAnchorPadding) {
        this.tooltipAnchorPadding = tooltipAnchorPadding;
    }
    ...
    public long mo889calculatePositionllwVHH4(IntRect anchorBounds, long windowSize, LayoutDirection layoutDirection, long popupContentSize) {
        ...
        if (IntSize.m5434getWidthimpl(popupContentSize) + x > IntSize.m5434getWidthimpl(windowSize) && (x = anchorBounds.getLeft() - IntSize.m5434getWidthimpl(popupContentSize)) < 0) {
            x = anchorBounds.getLeft() + ((anchorBounds.getWidth() - IntSize.m5434getWidthimpl(popupContentSize)) / 2);
        }
        int y = (anchorBounds.getTop() - IntSize.m5433getHeightimpl(popupContentSize)) - this.tooltipAnchorPadding;
        if (y < 0) {
            y = anchorBounds.getBottom() + this.tooltipAnchorPadding;
        }
