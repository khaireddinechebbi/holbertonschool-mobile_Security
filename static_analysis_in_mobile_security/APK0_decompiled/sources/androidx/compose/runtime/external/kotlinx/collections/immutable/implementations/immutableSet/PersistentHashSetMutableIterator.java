...
public final class PersistentHashSetMutableIterator<E> extends PersistentHashSetIterator<E> implements Iterator<E>, KMutableIterator {
    ...
    private final void resetPath(int hashCode, TrieNode<?> node, E element, int pathIndex) {
        if (isCollision(node)) {
            int index = ArraysKt.indexOf((E[]) node.getBuffer(), element);
            CommonFunctionsKt.m2616assert(index != -1);
            getPath().get(pathIndex).reset(node.getBuffer(), index);
            setPathLastIndex(pathIndex);
            return;
        }
        int position = 1 << TrieNodeKt.indexSegment(hashCode, pathIndex * 5);
        int index2 = node.indexOfCellAt$runtime_release(position);
        getPath().get(pathIndex).reset(node.getBuffer(), index2);
        Object cell = node.getBuffer()[index2];
        if (cell instanceof TrieNode) {
            resetPath(hashCode, (TrieNode) cell, element, pathIndex + 1);
        } else {
            setPathLastIndex(pathIndex);
        }
    }
