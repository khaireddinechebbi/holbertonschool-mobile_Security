...
public abstract class LazyGridMeasuredLineProvider {
    ...
    public final long m627childConstraintsJhjzzOo$foundation_release(int startSlot, int span) {
        int i;
        if (span != 1) {
        ...
        if (this.isVertical) {
            return Constraints.INSTANCE.m5228fixedWidthOenEA2s(crossAxisSize);
        }
        return Constraints.INSTANCE.m5227fixedHeightOenEA2s(crossAxisSize);
    }

    /* renamed from: itemConstraints-OenEA2s, reason: not valid java name */
    public final long m628itemConstraintsOenEA2s(int itemIndex) {
        LazyGridSpanLayoutProvider lazyGridSpanLayoutProvider = this.spanLayoutProvider;
        int span = lazyGridSpanLayoutProvider.spanOf(itemIndex, lazyGridSpanLayoutProvider.getSlotsPerLine());
        return m627childConstraintsJhjzzOo$foundation_release(0, span);
    }
    ...
    public final LazyGridMeasuredLine getAndMeasure(int lineIndex) {
        ...
        for (int i = 0; i < lineItemsCount; i++) {
            int span = GridItemSpan.m606getCurrentLineSpanimpl(lineConfiguration.getSpans().get(i).getPackedValue());
            long constraints = m627childConstraintsJhjzzOo$foundation_release(startSlot, span);
            LazyGridMeasuredItem m626getAndMeasure3p2s80s = this.measuredItemProvider.m626getAndMeasure3p2s80s(lineConfiguration.getFirstItemIndex() + i, mainAxisSpacing, constraints);
            startSlot += span;
            Unit unit = Unit.INSTANCE;
            items[i] = m626getAndMeasure3p2s80s;
        }
